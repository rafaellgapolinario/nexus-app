...
// ── TTS via Azure (server-side) ou Web Speech API (fallback) ──────────────────
export async function speakText(text: string, ttsEnabled: boolean): Promise<void> {
  ...
  // Fallback: Web Speech Synthesis nativo
  return new Promise(resolve => {
...
// ── Hook: STT robusto com auto-restart e timeout ──────────────────────────────
  ...
  const start = useCallback(() => {
    ...
    rec.onresult = (e: any) => {
      ...
      if (final) {
        ...
        // Silêncio de 1.5s após final → para
        silenceTimer.current = setTimeout(stop, 1500)
      }
    }

    rec.onerror = (e: any) => {
      if (e.error === 'no-speech') {
        // Auto-restart se não houve fala
        if (restartRef.current) { try { rec.stop() } catch {} }
        return
      }
    ...
    rec.onend = () => {
      // Auto-restart enquanto ativo (keep-alive)
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        letterSpacing: 4, color: 'var(--accent2)',
      ...
      <div style={{
        ...
        {[{ s: -10, op: 0.3, d: '2s', r: 'reverse' }, { s: -20, op: 0.15, d: '6s', r: 'normal' }].map((r, i) => (
          <div key={i} style={{
            position: 'absolute', inset: r.s, borderRadius: '50%',
            border: `1px ${i === 1 ? 'dashed' : 'solid'} rgba(124,109,250,${r.op})`,
            animation: `jarvisSpin ${r.d} linear infinite ${r.r}`,
          }} />
        ))}
        <div style={{
          width: 88, height: 88, borderRadius: '50%',
          background: 'radial-gradient(circle, rgba(124,109,250,0.25) 0%, rgba(124,109,250,0.08) 100%)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          animation: 'jarvisPulse 1.8s ease-in-out infinite',
        }}>
          <NexusIcon size={34} />
        </div>
      </div>
      ...
      <div style={{ fontSize: 15, color: 'var(--text)', fontWeight: 500, textAlign: 'center', maxWidth: 320 }}>
        {status}
      </div>
      ...
      <button
        ...
        style={{
          ...
          border: '1px solid rgba(248,113,113,0.25)', borderRadius: 99,
