    ...
    <AppShell>
      <div style={{ flex: 1, overflowY: 'auto', padding: '24px 32px 40px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: 'var(--text2)', marginTop: 4 }}>
              Março 2026 · {transactions.length} transações
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <Link href="/agent" style={{
              padding: '8px 14px', background: 'rgba(124,109,250,0.1)',
              border: '1px solid rgba(124,109,250,0.3)', borderRadius: 99,
              fontSize: 12, color: 'var(--accent2)', textDecoration: 'none', fontWeight: 600,
            }}>
              🎙️ Adicionar por voz
            </Link>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 20 }}>
          {[
            { label: 'Receitas', value: totalReceitas, color: 'var(--green)', prefix: '+' },
            { label: 'Gastos', value: totalGastos, color: 'var(--red)', prefix: '-' },
            { label: 'Saldo', value: Math.abs(saldo), color: saldo >= 0 ? 'var(--green)' : 'var(--red)', prefix: saldo >= 0 ? '+' : '-' },
            ...
            <div key={label} style={{
              ...
              <div style={{ fontFamily: 'Syne', fontSize: 26, fontWeight: 800, color }}>
        ...
        {/* Budget progress */}
        <div style={{
          background: 'var(--bg2)', border: `1px solid ${budgetUsed > 90 ? 'rgba(248,113,113,0.3)' : 'var(--border)'}`,
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 12, color: budgetUsed > 90 ? 'var(--red)' : 'var(--text2)' }}>
              R$ {totalGastos.toFixed(2)} / R$ {BUDGET_LIMIT.toLocaleString('pt-BR')} ({budgetUsed}%)
            </span>
          </div>
          <div style={{ height: 10, background: 'var(--bg3)', borderRadius: 99, overflow: 'hidden' }}>
            <div style={{
              ...
              background: budgetUsed > 90 ? 'var(--red)' : budgetUsed > 70 ? 'var(--amber)' : 'var(--green)',
              width: `${Math.min(100, budgetUsed)}%`,
            }} />
          </div>
          {budgetUsed > 90 && (
            <div style={{ fontSize: 12, color: 'var(--red)', marginTop: 8 }}>
              ⚠️ Você atingiu {budgetUsed}% do orçamento mensal!
            </div>
          )}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.6fr', gap: 20 }}>
          {/* Gastos por categoria */}
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1.2, color: 'var(--text3)', textTransform: 'uppercase', marginBottom: 12 }}>
              Gastos por categoria
            </div>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {(['todos', 'gasto', 'receita'] as const).map(f => (
                  <button key={f} onClick={() => setFilterType(f)} style={{
                    padding: '4px 10px', borderRadius: 99, border: `1px solid ${filterType === f ? 'var(--accent)' : 'var(--border)'}`,
                    background: filterType === f ? 'rgba(124,109,250,0.1)' : 'transparent',
                    color: filterType === f ? 'var(--accent2)' : 'var(--text3)',
                    cursor: 'pointer', fontSize: 11, fontWeight: filterType === f ? 600 : 400,
                  }}>{f}</button>
                ))}
              </div>
            </div>
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
                  {(['gasto', 'receita'] as const).map(t => (
                    <button key={t} onClick={() => setForm(f => ({ ...f, type: t }))} style={{
                      flex: 1, padding: '7px 0', borderRadius: 8, border: `1px solid ${form.type === t ? (t === 'gasto' ? 'var(--red)' : 'var(--green)') : 'var(--border)'}`,
                      background: form.type === t ? (t === 'gasto' ? 'rgba(248,113,113,0.1)' : 'rgba(34,211,160,0.1)') : 'transparent',
                      color: form.type === t ? (t === 'gasto' ? 'var(--red)' : 'var(--green)') : 'var(--text3)',
                      cursor: 'pointer', fontSize: 12, fontWeight: 600,
                    }}>{t === 'gasto' ? '💸 Gasto' : '💰 Receita'}</button>
                  ))}
                </div>
                <input value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                  placeholder="Descrição..." className="input-field" style={{ width: '100%', marginBottom: 8 }} />
                <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
                  <input value={form.value} onChange={e => setForm(f => ({ ...f, value: e.target.value }))}
                    placeholder="Valor (R$)" type="number" className="input-field" style={{ flex: 1 }} />
                  {form.type === 'gasto' && (
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button onClick={() => setShowAdd(false)} style={{
                    ...
                    borderRadius: 8, padding: 10, cursor: 'pointer', color: 'var(--text2)', fontSize: 13,
                  }}>Cancelar</button>
                </div>
              </div>
            )}
              ...
              {filtered.map(t => (
                <div key={t.id} style={{
                  ...
                  <div style={{
                    ...
                    color: t.type === 'receita' ? 'var(--green)' : 'var(--red)',
                  }}>
