      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '24px 32px 40px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <Link href="/agent" style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: 'rgba(124,109,250,0.1)', border: '1px solid rgba(124,109,250,0.3)', borderRadius: 99, fontSize: 12, color: 'var(--accent2)', textDecoration: 'none', fontWeight: 600 }}>🎙� Criar por voz</Link>
            <button onClick={() => setShowAdd(!showAdd)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: 'var(--accent)', border: 'none', borderRadius: 99, fontSize: 12, color: '#fff', cursor: 'pointer', fontWeight: 600 }}>+ Novo hábito</button>
          </div>
        </div>
        <div style={{ background: 'linear-gradient(135deg,rgba(34,211,160,0.1),rgba(34,211,160,0.04))', border: '1px solid rgba(34,211,160,0.2)', borderRadius: 'var(--radius)', padding: '16px 20px', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 16 }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 8 }}>Progresso hoje — {Math.round((doneTodayCount / habits.length) * 100)}%</div>
            <div style={{ height: 8, background: 'var(--bg3)', borderRadius: 99, overflow: 'hidden' }}>
              <div style={{ height: '100%', background: 'linear-gradient(90deg,var(--green),#4ade80)', borderRadius: 99, width: `${(doneTodayCount / habits.length) * 100}%`, transition: 'width 0.5s ease' }} />
            </div>
          </div>
          <div style={{ textAlign: 'center', flexShrink: 0 }}>
            <div style={{ fontFamily: 'Syne', fontSize: 28, fontWeight: 800, color: 'var(--green)' }}>{doneTodayCount}/{habits.length}</div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
          {(['todos', 'saúde', 'mente', 'foco'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)} style={{ padding: '6px 14px', borderRadius: 99, border: `1px solid ${filter === f ? 'var(--accent)' : 'var(--border)'}`, background: filter === f ? 'rgba(124,109,250,0.1)' : 'transparent', color: filter === f ? 'var(--accent2)' : 'var(--text2)', cursor: 'pointer', fontSize: 12, fontWeight: filter === f ? 600 : 400 }}>{f.charAt(0).toUpperCase() + f.slice(1)}</button>
          ))}
        </div>
