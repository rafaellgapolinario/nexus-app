...
export async function POST(req: NextRequest) {
  ...
  const ssml = `<speak version='1.0' xml:lang='pt-BR'><voice name='pt-BR-JulioNeural'><prosody rate='+3%' pitch='-0.2st'>${escaped}</prosody></voice></speak>`
  try {
    ...
    const ttsRes = await fetch(`https://${region}.tts.speech.microsoft.com/cognitiveservices/v1`, { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/ssml+xml', 'X-Microsoft-OutputFormat': 'audio-24khz-48kbitrate-mono-mp3' }, body: ssml })
