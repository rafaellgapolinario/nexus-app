...
export default function NexusPage() {
  ...
  const [msgs, setMsgs] = useState<Msg[]>([{id:uid(),role:'nexus',time:tstr(),
    text:`Olá${userProfile?', '+(userProfile.given_name||userProfile.name):''}! Estou ouvindo. Fale comigo!`}])
  ...
  const saveMsg = useCallback(async (role:'user'|'assistant', content:string)=>{
    if(!userProfile?.email) return
    try {
      // Get or create conversation
      let convId = sessionStorage.getItem('nexus_conv_id')
      if(!convId) {
        const res = await fetch('/api/conversations',{
          method:'POST',
          headers:{'Content-Type':'application/json','x-user-email':userProfile.email},
          body:JSON.stringify({titulo:`Conversa ${new Date().toLocaleDateString('pt-BR')}`})
        })
        if(res.ok){ const d=await res.json(); convId=d.id; sessionStorage.setItem('nexus_conv_id',convId||'') }
      }
      if(convId) {
        await fetch('/api/conversations/messages',{
          method:'POST',
          headers:{'Content-Type':'application/json','x-user-email':userProfile.email},
          body:JSON.stringify({conversation_id:convId, role, conteudo:content})
        })
      }
    } catch {}
  },[userProfile?.email])
  ...
  const startListen = useCallback(()=>{
    ...
    if(!SR||lisRef.current||['thinking','speaking'].includes(sRef.current)) return
    ...
    setTimeout(()=>{
      ...
      rec.onresult=(e:any)=>{
        ...
        if(e.results[e.results.length-1].isFinal){
          ...
          else setTimeout(startListen,500) // noise — restart
        }
      }
      rec.onerror=(e:any)=>{
        stopListen()
        if(e.error==='not-allowed') showToast('Permita o microfone nas configurações do Chrome.')
        else if(e.error!=='aborted') setTimeout(startListen,1500)
      }
  ...
  const sendToAI = useCallback(async (text:string)=>{
    ...
    try {
      const res = await fetch('/api/chat',{
        ...
        body:JSON.stringify({ messages:[...chatHistory,{role:'user',content:text}], userName:userProfile?.given_name||userProfile?.name, lang, calendarContext:cal, geminiKey, voiceMode:true, userEmail:userProfile?.email, accessToken })
      })
      const data = await res.json()
      const reply = data.reply||'Não consegui processar.'
      let tag=''
      if(data.noteCreated){ tag='📝 Nota salva!'; if(data.noteData&&userProfile?.email){
        fetch('/api/notes',{method:'POST',headers:{'Content-Type':'application/json','x-user-email':userProfile.email},
          body:JSON.stringify({tipo:data.noteData.tipo||'nota',titulo:data.noteData.titulo||null,conteudo:data.noteData.conteudo||'',lembrete_em:data.noteData.lembrete_em||null,cor:data.noteData.tipo==='tarefa'?'#22d3a0':data.noteData.tipo==='lembrete'?'#f59e0b':'#7c6dfa'})
        }).catch(()=>{})
      }}
      ...
      // Always restart after speaking — always-on mode
      speak(reply,()=>{ setTimeout(startListen,600) })
    } catch {
      const err='Erro de conexão.'
      setMsgs(p=>[...p,{id:uid(),role:'nexus',text:err,time:tstr()}])
      speak(err,()=>{ setTimeout(startListen,1000) })
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[calendarEvents,chatHistory,userProfile,lang,geminiKey,accessToken,speak,addMessage,showToast,saveMsg])

  // ── Wake word (backup for when mic is idle) ────────────
  ...
  useEffect(()=>{
    const t=setTimeout(()=>startListen(),1000)
    return ()=>{
      clearTimeout(t)
      try{wakeRef.current?.stop()}catch{}
      try{recRef.current?.stop()}catch{}
  ...
  // Restart after idle (always-on)
  useEffect(()=>{
    if(s==='idle'){
      const t=setTimeout(()=>{ if(sRef.current==='idle'&&!lisRef.current) startListen() },800)
  ...
  useEffect(()=>{
    const onKey=(e:KeyboardEvent)=>{
      ...
      if(e.code==='Escape'){
        stopListen(); audioRef.current?.pause(); window.speechSynthesis?.cancel(); setS('idle')
      }
    }
  ...
  const statusLabel = { idle:'🎙️ Ouvindo...', listening:'🔴 Captando voz...', thinking:'⚙️ Pensando...', speaking:'🔊 Falando...' }[s]
  ...
  return (
    <AppShell>
      <div style={{flex:1,display:'flex',flexDirection:'column',height:'100%',background:'var(--bg)',overflow:'hidden',position:'relative'}}>
        ...
        {/* Ambient */}
        <div style={{position:'absolute',inset:0,pointerEvents:'none',background:`radial-gradient(ellipse 70% 50% at 50% 50%,${col}0d 0%,transparent 70%)`,transition:'background 0.6s',zIndex:0}}/>
        ...
        <div style={{padding:'12px 24px',borderBottom:'1px solid var(--border)',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0,zIndex:1,position:'relative'}}>
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            <div style={{width:8,height:8,borderRadius:'50%',background:col,boxShadow:`0 0 8px ${col}`,animation:'pulse 2s infinite',transition:'background 0.3s'}}/>
            <span style={{fontFamily:'Syne',fontSize:15,fontWeight:700}}>Nexus · Assistente</span>
            <span style={{fontSize:11,color:'var(--text3)',background:'var(--bg3)',padding:'2px 8px',borderRadius:99,border:'1px solid var(--border)'}}>sempre ativo</span>
          </div>
        ...
        {/* HUD when active */}
        {isActive&&(
          <div style={{position:'absolute',top:'50%',left:'50%',transform:'translate(-50%,-58%)',zIndex:1,pointerEvents:'none',display:'flex',flexDirection:'column',alignItems:'center',gap:16}}>
            <div style={{position:'relative',width:180,height:180,display:'flex',alignItems:'center',justifyContent:'center'}}>
              {[160,130,100].map((sz,i)=>(
                <div key={i} style={{position:'absolute',width:sz,height:sz,borderRadius:'50%',border:`1.5px solid ${col}${['22','44','77'][i]}`,animation:`ring ${1.4+i*0.3}s ${i*0.1}s ease-in-out infinite`}}/>
              ))}
              <div style={{width:72,height:72,borderRadius:'50%',background:`radial-gradient(circle at 38% 38%,${col}ee,${col}55)`,boxShadow:`0 0 36px ${col}99,0 0 72px ${col}44`,display:'flex',alignItems:'center',justifyContent:'center',animation:'corePulse 2s ease-in-out infinite'}}>
                {s==='listening'&&<svg width="26" height="26" fill="none" viewBox="0 0 24 24" stroke="white" strokeWidth="1.8"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="23"/></svg>}
                {s==='thinking'&&<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2"><path d="M21 12a9 9 0 1 1-6.219-8.56" strokeLinecap="round"/></svg>}
                {s==='speaking'&&<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.8"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/></svg>}
              </div>
            </div>
            {live&&<div style={{fontSize:13,color:'rgba(255,255,255,0.65)',maxWidth:280,textAlign:'center',fontStyle:'italic',padding:'6px 14px',background:'rgba(0,0,0,0.3)',borderRadius:99,backdropFilter:'blur(8px)'}}>{live}</div>}
            <div style={{fontSize:11,color:col,fontWeight:700,letterSpacing:2,textTransform:'uppercase'}}>{statusLabel}</div>
          </div>
        )}
            ...
            <div key={m.id} style={{display:'flex',flexDirection:'column',alignItems:m.role==='user'?'flex-end':'flex-start',gap:4}}>
              {m.role==='nexus'&&(
                <div style={{display:'flex',alignItems:'center',gap:6,marginLeft:4}}>
                  ...
                  {m.tag&&<span style={{fontSize:10,color:'var(--green)',background:'rgba(34,211,160,0.1)',padding:'1px 7px',borderRadius:99,border:'1px solid rgba(34,211,160,0.2)'}}>{m.tag}</span>}
                </div>
              )}
        ...
        <div style={{flexShrink:0,padding:'12px 24px 18px',borderTop:'1px solid var(--border)',background:'var(--bg)',zIndex:1,position:'relative'}}>
          <div style={{height:24,display:'flex',gap:2,alignItems:'center',justifyContent:'center',marginBottom:10,opacity:isActive?1:0.12,transition:'opacity 0.4s'}}>
            {Array.from({length:22}).map((_,i)=>(
              <div key={i} style={{width:3,borderRadius:99,background:col,boxShadow:isActive?`0 0 3px ${col}`:undefined,animation:isActive?`wave ${0.35+Math.sin(i)*0.18}s ${i*0.04}s ease-in-out infinite`:'none',height:4,transition:'background 0.3s'}}/>
            ))}
          </div>
          <div style={{display:'flex',gap:8,alignItems:'flex-end'}}>
            ...
            />
            <button onClick={handleMic} style={{width:50,height:50,borderRadius:'50%',border:'none',cursor:'pointer',flexShrink:0,background:`radial-gradient(circle at 38% 38%,${col},${col}88)`,boxShadow:isActive?`0 0 0 5px ${col}33,0 0 24px ${col}66`:`0 4px 14px ${col}55`,display:'flex',alignItems:'center',justifyContent:'center',transition:'all 0.25s',animation:s==='listening'?'micPulse 1.2s ease-in-out infinite':'none'}}>
              {s==='speaking'?<svg width="18" height="18" viewBox="0 0 24 24" fill="white"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02z"/></svg>
              :s==='thinking'?<div style={{width:18,height:18,border:'2px solid rgba(255,255,255,0.3)',borderTopColor:'#fff',borderRadius:'50%',animation:'spin 0.8s linear infinite'}}/>
              :<svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="white" strokeWidth="2"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg>}
            </button>
          ...
          <div style={{display:'flex',gap:6,marginTop:8,flexWrap:'wrap'}}>
            {[['📅 Agenda','O que tenho na agenda hoje?'],['📝 Anota','Anota: '],['✅ Tarefa','Cria tarefa: '],['⏰ Lembrete','Me lembra de '],['🧠 Semana','Resumo da minha semana']].map(([l,v])=>(
              <button key={l} onClick={()=>{ if((v).endsWith(': ')||(v).endsWith('de ')){ setInput(v) }else{ sendToAI(v) } }}
          ...
          <div style={{textAlign:'center',marginTop:6,fontSize:10,color:'var(--text3)'}}>
            <kbd style={{background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:3,padding:'1px 5px'}}>Esc</kbd> parar ·
            <kbd style={{background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:3,padding:'1px 5px',margin:'0 3px'}}>Space</kbd> forçar ouvir · sempre ativo
          </div>
        </div>
      </div>
      <style>{`
        ...
        @keyframes micPulse  { 0%,100%{box-shadow:0 0 0 0 rgba(248,113,113,0.5)} 50%{box-shadow:0 0 0 12px rgba(248,113,113,0)} }
      `}</style>
    </AppShell>
  )
}
